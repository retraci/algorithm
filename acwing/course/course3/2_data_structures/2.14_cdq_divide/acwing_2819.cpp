...
void solve() {
    ...
    for (auto &[x, y, z] : ps) z = n - z + 1;

    for (auto &[x, y, z] : ps) x = -x;
    auto ret1 = cdq.work(ps, fw, n);

    for (auto &[x, y, z] : ps) x = -x, y = -y;
    auto ret2 = cdq.work(ps, fw, n);

    vector<ll> ans(n + 1);
    for (auto [res, x, y, z, cnt] : ret1) {
        int rz = n - z + 1;
        ans[rz] += res;
    }
    for (auto [res, x, y, z, cnt] : ret2) {
        int rz = n - z + 1;
        ans[rz] += res;
    }
